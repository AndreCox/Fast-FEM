...
void GraphicsRenderer::drawGrid(sf::RenderWindow &window) const
{
    ...
    // Grid spacing in world units (internal units are meters)
    float gridSpacing = 1.0f; // default 1 meter
    // Choose spacing based on unit system so grid matches 1x1 m / ft / in
    switch (system.unit_system)
    {
    case Metric:
        gridSpacing = 1.0f; // 1 meter
        break;
    case ImperialFeet:
        gridSpacing = 0.3048f; // 1 foot = 0.3048 m
        break;
    case ImperialInches:
        gridSpacing = 0.0254f; // 1 inch = 0.0254 m
        break;
    default:
        gridSpacing = 1.0f;
        break;
